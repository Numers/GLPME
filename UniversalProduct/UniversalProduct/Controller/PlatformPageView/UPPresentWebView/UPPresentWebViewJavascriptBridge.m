//
//  UPPresentWebViewJavascriptBridge.m
//  UniversalProduct
//
...
static BOOL didLogout = NO;
@implementation UPPresentWebViewJavascriptBridge
-(void)registerEvent
{
    [self registerHandler:@"closeWebView" handler:^(id data, WVJBResponseCallback responseCallback) {
        if ([self.delegate respondsToSelector:@selector(closeWebView:)]) {
            [self.delegate closeWebView:^(BOOL success) {
                if (success) {
                    [self callbackSuccess:@"关闭成功" data:nil callback:responseCallback];
                }else{
                    [self callbackFailed:@"当前并不是web页" callback:responseCallback];
                }
            }];
        }
